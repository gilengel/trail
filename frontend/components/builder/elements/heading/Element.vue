...
watch(
  () => props.element.properties.color,
  () => {
    if (!textEditor.value) {
      return;
    }
...
watch(
  () => props.element.properties.level,
  () => {
    if (!textEditor.value) {
      return;
    }
...
function onTextChanged(newContent: string) {
  ...
  const document: Document = parser.parseFromString(
    newContent,
    "text/html",
  ) as Document;
  ...
  editor?.executeAction(
    new UpdateElementAttribute<typeof HeadingElement>(
      props.element,
      "text",
      element.textContent!,
    ),
