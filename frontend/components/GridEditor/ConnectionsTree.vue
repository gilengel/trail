<template>
  ...
  >
    <template v-slot:prepend="{ item, depth }">
      <v-icon v-if="depth" :icon="item.definition!.metadata!.icon!"></v-icon>

    </template>
    <template v-slot:append="{ item, depth }">
      <v-icon-btn
          v-if="depth"
          :disabled="!depth"
...
<script setup lang="ts" generic="ElementDefinition extends EditorElementDefinition">
// ---------------------------------------------------------------------------------------------------------------------
...
const items: Ref<EventTreeItemType[]> = computed(() => {
  ...
  const convertChildren = (eventId: string) => {
    const children = element.connections.provided.events.listeners[eventId].map(child => {
      const definition = editor.definitions.get(editor.instances.getInstance(child)?.elementId!)!;
