...
<script
  setup
  lang="ts"
  generic="Element extends EditorElementDefinition<any, any, any, any>"
>
// ---------------------------------------------------------------------------------------------------------------------
...
const emit =
  defineEmits<
    (e: "connectedConsumedPropertyRemoved", property: string) => void
  >();
...
if (!editor) {
  throw new Error(
    "Container component is missing the 'editor' injected object.",
  );
}
...
const provided = computed(() => {
  function fn(property: string) {
    return {
      property,
      connected:
        (props.element.connections.provided as Record<string, unknown>)[
          property
        ] !== undefined,
    };
  }
...
const consumed = computed(() => {
  function fn(property: string) {
    return {
      property,
      connected:
        (props.element.connections.consumed as Record<string, unknown>)[
          property
        ] !== undefined,
    };
  }
 ...
 */
function findAllElementsWithProperties(
  propertyKeys: string[],
  grid: Grid,
  direction: Direction,
): EditorElementInstance<any>[] {
  function otherElementHasAllPropertiesAsConnections(
    _propertyKeys: string[],
  ...
  const otherElementThanCurrentInstances = (element: EditorElementInstance) =>
      ...
      row.columns
        ...
        .filter(
          (element) =>
            ...
            otherElementHasAllPropertiesAsConnections(
              propertyKeys,
              getCorrectProperties(element),
            ),
        )
...
function propertySelected(propertyKey: string, direction: Direction) {
  const inverseDirection =
    direction === Direction.Consumed ? Direction.Provided : Direction.Consumed;
  const filtered = findAllElementsWithProperties(
    [propertyKey],
    props.grid,
    inverseDirection,
  );
