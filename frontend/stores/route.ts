...
export const useRouteStore = () =>
  defineStore("routeStore", () => {
    ...
    const convertedRoutesByTripId = reactive(
      new Map<number, MapLibreRoute[]>(),
    );
    ...
    async function getByRouteId(routeId: number): Promise<RouteDto | null> {
      ...
      if (dtoRoutesByTripId.has(route.tripId)) {
        const existingRoutesPerTrip: RouteDto[] = dtoRoutesByTripId.get(
          route.tripId,
        )!;
        const existing: RouteDto | undefined = existingRoutesPerTrip.find(
    ...
    async function getByTripId(tripId: number): Promise<RouteDto[] | null> {
      ...
      const updatedRoutes = await Promise.all(
        routeDTOs.value.map(async (route: RouteDto) => {
          const segments = await $fetch<RouteSegmentDto[]>(
            `/api/routes/segment/route/${route.id}`,
          );
     ...
     * @returns Promise with the trip.
     */
    async function getMapLibreRoutesForTrip(
      tripId: number,
    ): Promise<MapLibreRoute[] | null> {
     ...
     */
    async function getMapLibreRoute(
      routeId: number,
    ): Promise<MapLibreRoute | null> {
