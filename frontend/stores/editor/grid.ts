...
import {GroupedUndoRedoAction, UndoRedoHandler} from "./undoredo";


import {type Grid, Element, type Column, type Row} from "~/components/GridEditor/grid";
...
export class GridHandler {
  ...
  constructor(private _undoRedoStore: UndoRedoHandler) {
  }
   ...
   */
  public async addRow(row: Row, grid: Grid) {
    await this._undoRedoStore.execute(new AddRow(row, grid));
  }
   ...
   */
  public async deleteRow(rowIndex: number, grid: Grid) {
    const row = grid.rows[rowIndex];
    await this._undoRedoStore.execute(new DeleteRow(row!, grid));
  }
   ...
   */
  public async deleteColumn(rowIndex: number, columnIndex: number, grid: Grid) {
    const row = grid.rows[rowIndex]!;

    await this._undoRedoStore.execute(new DeleteColumn(row, columnIndex, grid));
  }
   ...
   */
  public async splitColumn(rowIndex: number, columnIndex: number, grid: Grid) {
    const row = grid.rows[rowIndex]!;

    await this._undoRedoStore.execute(new SplitColumn(row, columnIndex, grid));
  }
   ...
   */
  public async moveRow(oldRowIndex: number, newRowIndex: number, grid: Grid) {
    await this._undoRedoStore.execute(
   ...
   */
  public async setColumnElement(column: Column, element: Element<any, any, any>) {
    await this._undoRedoStore.execute(new SetElement(column, element));
  }
   ...
   */
  public async updateColumnsWidth(
