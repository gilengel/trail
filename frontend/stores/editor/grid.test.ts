...
import {createDefaultGrid, useGridStore} from "~/stores/editor/grid";
...
describe('GridModuleStore', () => {
    ...
    let undoRedoStoreMock: ReturnType<ReturnType<typeof useUndoRedoStore>>;

    beforeEach(() => {
        setActivePinia(createPinia());
        undoRedoStoreMock = useUndoRedoStore(vi.fn())();
        store = useGridStore(undoRedoStoreMock);
    ...
    it('calls execute with DeleteColumn when deleting a column', async () => {
        ...
        expect(undoRedoStoreMock.execute).toHaveBeenCalledWith(new DeleteColumn(grid.rows[0], 0, grid));
    });
    ...
    it('calls execute with SplitColumn when splitting a column', async () => {
        ...
        expect(undoRedoStoreMock.execute).toHaveBeenCalledWith(new SplitColumn(grid.rows[0], 0, grid));
    });
