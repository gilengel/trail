...
import { MapLibreTrip, MapLibreSegment } from "./types";
import L from "leaflet";

const route = new MapLibreTrip(
  0,
  "single route",
  [
    new MapLibreSegment(
...
const emptyRoute = new MapLibreTrip(0, "single route", [], "some description");
const emptySegment = new MapLibreSegment(0, "", [], "", "");
...
describe("LeafletRoute", () => {
  ...
  it("should get the start coordinate", () => {
    expect(route.start).to.be.toStrictEqual(
      L.latLng(47.387563828378916, 10.939971758052707)
    );
  });
  ...
  it("should get the end coordinate", () => {
    expect(route.end).to.be.toStrictEqual(
      L.latLng(27.387563828378916, 10.939971758052707)
    );
  });
...
describe("LeafletSegment", () => {
  ...
  it("should get polyline", () => {
    expect(route.segments[0].polyline).to.be.toStrictEqual(
      L.polyline(route.segments[0].coordinates, {
