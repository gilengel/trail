...
export namespace DB {
    ...
    export type RouteSegmentCreateInput = Partial<Omit<RouteSegment, "routeId">> & { routeId: number }
    export type RouteSegmentUpdateInput = Pick<Prisma.RouteSegmentUpdateInput, "name" | "description"> & Partial<Coordinates>
}
...
export class RouteSegmentsDatabase {
    ...
    async create(segment: DTO.CreateRouteSegmentPrivate, route: DTO.Route): Promise<DTO.RouteSegment | null> {
        const coordinatesInWktFormat = numberArray2wkt(segment.coordinates)

        const insertedRouteSegmentWKT = await this.prisma.$queryRaw<DB.RouteSegment[]>`
    ...
    async createMultiple(segments: DTO.CreateRouteSegmentPrivate[], route: DTO.Route): Promise<DTO.RouteSegment[]> {
        ...
        const insertedRouteSegments =
            await this.prisma.$queryRaw<DB.RouteSegment[]>`
            INSERT INTO "RouteSegment"
                ("routeId", "name", "description", "coordinates")
    ...
    async getAllForRoute(route: DTO.Route): Promise<DTO.RouteSegment[]> {
        ...
        const segment = segmentsWKT.map((segment) =>
            this.db2Dto(segment),
        );
     ...
     */
    async update(id: number, data: DTO.UpdateRouteSegment): Promise<DTO.RouteSegment> {
        ...
        // Coordinates only
        if (data.coordinates) {
            const coordinates = numberArray2wkt(
                data.coordinates,
            );
